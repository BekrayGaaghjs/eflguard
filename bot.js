...
//  banlamaSınırı: 1, //Yasaklanmadan önce aralıkta gönderilmesine izin verilen maksimum ileti miktar.
 // aralık: 20000, // ms kullanıcılarda zaman miktarı, yasaklanmadan önce aralık değişkeninin maksimumunu gönderebilir.
...
client.on("message", async message => {
  ...
  if (lus) {
    const reklamengel = ["amk", "oç", "orrrrrrrrrrr","siktirgit","sik","am","ananısikeyim","orospucocugu  "  ];
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('KICK_MEMBERS')) {
          message.delete();
